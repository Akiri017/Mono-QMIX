  ...
  return (
    <div className={className} style={{
      background: isDark
        ...
        : 'linear-gradient(155deg, rgba(255,255,255,0.92) 0%, rgba(255,255,255,0.72) 100%)',
      ...
      boxShadow: isDark
        ...
        : 'inset 0 1px 0 rgba(255,255,255,0.8), 0 8px 32px rgba(15,23,42,0.08)',
      ...style,
    }}>{children}</div>
  )
}
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  const { theme } = useTheme()
  const isDark = theme === 'dark'
  const accent = isDark ? '#06B6D4' : '#0284C7'
  return (
    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full mb-4"
      style={{
        background: isDark ? 'rgba(6,182,212,0.1)' : 'rgba(2,132,199,0.08)',
        border: `1px solid ${isDark ? 'rgba(6,182,212,0.25)' : 'rgba(2,132,199,0.2)'}`,
      }}>
      <div className="w-1.5 h-1.5 rounded-full" style={{ background: accent }} />
      <span className="text-[10px] font-bold tracking-widest uppercase" style={{ color: accent }}>{children}</span>
    </div>
  )
}
...
function ThemeToggle() {
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLElement).style.background = isDark ? 'rgba(255,255,255,0.14)' : 'rgba(15,23,42,0.12)'
        ;(e.currentTarget as HTMLElement).style.color = isDark ? '#fff' : '#0f172a'
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLElement).style.background = isDark ? 'rgba(255,255,255,0.08)' : 'rgba(15,23,42,0.07)'
        ;(e.currentTarget as HTMLElement).style.color = isDark ? 'rgba(255,255,255,0.6)' : '#475569'
      }}
    >
      {isDark ? (
        // Sun icon
...
const StatusBar = () => {
  ...
  return (
    <div className="flex items-center justify-between px-7 py-2.5 flex-shrink-0"
      style={{
        background: isDark ? 'rgba(0,0,0,0.35)' : 'rgba(248,252,255,0.7)',
        borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(15,23,42,0.08)'}`,
      }}>
      <div className="flex items-center gap-2.5">
        <img src="/icons/civiq-logo.png" alt="Civiq" className={`w-5 h-5 object-contain ${isDark ? 'brightness-0 invert opacity-80' : 'opacity-80'}`} />
        <span className="font-bold text-[13px] tracking-widest" style={{ color: isDark ? 'rgba(255,255,255,0.75)' : '#0f172a' }}>CIVIQ</span>
        <span className="text-[10px] font-medium" style={{ color: isDark ? 'rgba(255,255,255,0.3)' : '#94a3b8' }}>
      ...
      <div className="flex items-center gap-1">
        {NAV_LINKS.map(({ label, href }) => (
          <button key={label} onClick={() => router.push(href)}
            ...
            style={{ color: isDark ? 'rgba(255,255,255,0.52)' : '#475569', background: 'transparent' }}
            onMouseEnter={(e) => {
              ;(e.currentTarget as HTMLElement).style.color = isDark ? 'rgba(255,255,255,0.9)' : '#0f172a'
              ;(e.currentTarget as HTMLElement).style.background = isDark ? 'rgba(255,255,255,0.07)' : 'rgba(15,23,42,0.06)'
            }}
            onMouseLeave={(e) => {
              ;(e.currentTarget as HTMLElement).style.color = isDark ? 'rgba(255,255,255,0.52)' : '#475569'
...
export default function Home() {
  ...
  // ── Color tokens ────────────────────────────────────────────────────────────
  const c = {
    pageBg:         isDark ? '#060112'                : '#f0f6ff',
    pageBgGrad:     isDark ? 'linear-gradient(135deg, #060112 0%, #0b0320 40%, #040c1c 100%)' : 'linear-gradient(135deg, #f0f6ff 0%, #e8f0fe 40%, #f0f8ff 100%)',
    heroBg:         isDark ? 'rgba(8,14,32,0.48)'    : 'rgba(255,255,255,0.55)',
    heroInner:      isDark ? 'rgba(6,11,26,0.62)'    : 'rgba(248,252,255,0.82)',
    heroBorder:     isDark ? 'rgba(255,255,255,0.11)': 'rgba(15,23,42,0.1)',
    heroShadow:     isDark ? '0 32px 80px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.14), inset 0 -1px 0 rgba(0,0,0,0.3)' : '0 32px 80px rgba(15,23,42,0.1), inset 0 1px 0 rgba(255,255,255,0.9)',
    heroInnerBorder:isDark ? 'rgba(255,255,255,0.08)': 'rgba(15,23,42,0.08)',
    heroInsetTop:   isDark ? 'rgba(255,255,255,0.05)': 'rgba(255,255,255,0.6)',
    heroDivider:    isDark ? 'rgba(255,255,255,0.09)': 'rgba(15,23,42,0.08)',
    heroCtrlBg:     isDark ? 'rgba(255,255,255,0.04)': 'rgba(255,255,255,0.6)',
    heroCtrlBorder: isDark ? 'rgba(255,255,255,0.08)': 'rgba(15,23,42,0.1)',
    heroCtrlInset:  isDark ? 'rgba(255,255,255,0.05)': 'rgba(255,255,255,0.7)',
    textH1Grad:     'linear-gradient(140deg, #93C5FD 0%, #60A5FA 40%, #38BDF8 100%)',
    textBody:       isDark ? 'rgba(255,255,255,0.52)' : '#475569',
    textPrimary:    isDark ? 'rgba(255,255,255,0.88)' : '#0f172a',
    textSecondary:  isDark ? 'rgba(255,255,255,0.55)' : '#334155',
    textMuted:      isDark ? 'rgba(255,255,255,0.35)' : '#64748b',
    textUltraMuted: isDark ? 'rgba(255,255,255,0.25)' : '#94a3b8',
    accent:         isDark ? '#38BDF8' : '#0284C7',
    accentLight:    isDark ? '#06B6D4' : '#0284C7',
    sectionBorder:  isDark ? 'rgba(255,255,255,0.06)' : 'rgba(15,23,42,0.08)',
    footerBg:       isDark ? 'rgba(0,0,0,0.25)'       : 'rgba(240,246,255,0.8)',
    emailDefault:   isDark ? 'rgba(255,255,255,0.3)'  : '#94a3b8',
    emailHover:     isDark ? '#38BDF8'                : '#0284C7',
    socialBg:       isDark ? 'rgba(255,255,255,0.06)' : 'rgba(15,23,42,0.06)',
    socialBorder:   isDark ? 'rgba(255,255,255,0.1)'  : 'rgba(15,23,42,0.1)',
    socialColor:    isDark ? 'rgba(255,255,255,0.45)' : '#64748b',
    socialBgHover:  isDark ? 'rgba(6,182,212,0.12)'   : 'rgba(2,132,199,0.1)',
    dotBg:          isDark ? 'linear-gradient(135deg,rgba(255,255,255,0.18),rgba(255,255,255,0.04))' : 'linear-gradient(135deg,rgba(15,23,42,0.12),rgba(15,23,42,0.04))',
    dotShadow:      isDark ? 'inset 0 1px 2px rgba(0,0,0,0.6)' : 'inset 0 1px 2px rgba(15,23,42,0.1)',
  }
    ...
    <main className="w-full" style={{ background: c.pageBg }}>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden"
        ...
        <div className="relative w-full mx-auto" style={{
          ...
          background: c.heroBg, backdropFilter: 'blur(32px)', WebkitBackdropFilter: 'blur(32px)',
          ...
          <div className="relative w-full flex flex-col overflow-hidden" style={{
            background: c.heroInner, backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)',
            ...
            <div className="flex flex-col items-center gap-5"
              ...
              <div className="text-center w-full" style={{ maxWidth: '780px' }}>
                ...
                <div className="flex gap-3 justify-center">
                  <a href="/simulation?mapSize=2km&trafficScale=stable_flow&view=focused&algorithm1=hierarchical_qmix"
              ...
              <div className="w-full" style={{
                ...
                boxShadow: `inset 0 1px 0 ${c.heroCtrlInset}`, padding: 'clamp(16px, 2vw, 24px)',
              }}>
              ...
              {[
                {
                  ...
                  sub: 'Consistent across free-flow to forced-flow',
                  color: '#38BDF8', colorDim: 'rgba(56,189,248,0.1)', border: 'rgba(56,189,248,0.25)',
                  icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>,
                },
                {
                  value: '2,273', unit: 'veh/hr',
                  label: 'Peak Network Throughput',
                  sub: '37.4% more than QMIX at max congestion',
                  color: '#A78BFA', colorDim: 'rgba(167,139,250,0.1)', border: 'rgba(167,139,250,0.25)',
                  icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>,
                },
                {
                  value: '19.3', unit: 'sec',
                  label: 'Wait Time at Peak Load',
                  sub: '18.8% less waiting than QMIX under congestion',
                  color: '#4ADE80', colorDim: 'rgba(74,222,128,0.1)', border: 'rgba(74,222,128,0.25)',
                ...
                const rgb = color === '#38BDF8' ? '56,189,248' : color === '#A78BFA' ? '167,139,250' : '74,222,128'
                return (
                  <GlassCard key={label} className="p-6" style={{ border: `1px solid ${border}`, background: colorDim }}>
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ background: `rgba(${rgb},0.12)`, color, border: `1px solid ${border}` }}>Civiq</span>
                    </div>
            ...
            <div className="grid grid-cols-2 gap-8 items-start">
              <div className="space-y-4">
                ...
                <p className="text-[14px] leading-relaxed" style={{ color: c.textSecondary }}>
                  ...
                  <span className="font-semibold" style={{ color: '#4ADE80' }}>9.7% less CO₂</span> and{' '}
                  <span className="font-semibold" style={{ color: '#4ADE80' }}>9.8% less fuel consumption</span> compared to Monolithic QMIX — demonstrating that hierarchical coordination is both faster and greener under load.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {[
                  { value: '493', unit: 'g/km', label: 'Avg. CO₂ Emissions', pct: 62, color: '#4ADE80' },
                  { value: '21.2', unit: 'l/100km', label: 'Avg. Fuel Consumption', pct: 70, color: '#38BDF8' },
                    ...
                    <GlassCard key={label} className="p-5 flex flex-col items-center gap-3">
                      <div className="relative w-32 h-32">
                        <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
                          <circle cx="60" cy="60" r={r} fill="none" stroke={isDark ? 'rgba(255,255,255,0.08)' : 'rgba(15,23,42,0.08)'} strokeWidth="10" />
      ...
      {/* ── RESEARCHERS ── */}
      <section id="researchers" className="relative w-full py-24" style={{ zIndex: 2, borderTop: `1px solid ${c.sectionBorder}` }}>
      ...
      {/* ── FOOTER ── */}
      <footer id="contact" className="relative w-full py-14" style={{ zIndex: 2, borderTop: `1px solid ${c.sectionBorder}`, background: c.footerBg }}>
        <div className="max-w-5xl mx-auto px-8">
          <div className="flex items-start justify-between gap-12 mb-12">
            {/* Brand */}
            <div className="max-w-xs">
              <div className="flex items-center gap-2.5 mb-3">
                <img src="/icons/civiq-logo.png" alt="Civiq" className={`w-6 h-6 object-contain ${isDark ? 'brightness-0 invert opacity-75' : 'opacity-80'}`} />

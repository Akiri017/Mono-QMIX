#!/usr/bin/env python3
"""
Block A — BGC Core LOS E Data Collection

Runs a headless 3600-step SUMO simulation on BGC Core at LOS E (trips_high.xml)
using libsumo. Logs all vehicle positions every timestep and outputs:

  vehicle_positions_bgc_core_los_e.csv   — timestep, vehicle_id, x, y
  vehicle_density_bgc_core_los_e.png     — 2D spatial heatmap (all timesteps)

Peak simultaneous vehicle count is printed to stdout.

Usage (from repo root):
    python scripts/bgc/block_a_bgc_core.py
...
BGC_DIR    = REPO_ROOT / "bgc_core"
NET_FILE   = BGC_DIR / "final_map.net.xml"
TRIPS_FILE = BGC_DIR / "trips_high.xml"
VTYPES_FILE = BGC_DIR / "vtypes.add.xml"

OUT_DIR    = SCRIPT_DIR                      # outputs land beside this script
CSV_PATH   = OUT_DIR / "vehicle_positions_bgc_core_los_e.csv"
PNG_PATH   = OUT_DIR / "vehicle_density_bgc_core_los_e.png"
...
def main():
    ...
    print("BGC Core LOS E — data collection run")
    ...
    # ------------------------------------------------------------------
    # Import libsumo (same backend used by the 4x4 environment)
    # ------------------------------------------------------------------
    try:
    ...
    # ------------------------------------------------------------------
    # Simulation loop — collect positions every step
    # ------------------------------------------------------------------
    ...
    h = ax.hist2d(all_x, all_y, bins=120, cmap="hot_r")
    plt.colorbar(h[3], ax=ax, label="Vehicle-timestep count")

    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.set_title(
        f"BGC Core Vehicle Density — LOS E ({EPISODE_STEPS} steps)\n"
